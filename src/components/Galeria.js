...
export default function Home() {
  const { fotos, setFotos, favoritas, setFavoritas } = useContext(MyContext);
  ...
  function favoritos(id) {
    ...
    if (elemento.style.color === "red") {
      ...
      fotos.map((foto) =>
        foto.id == id ? setFavoritas([...favoritas, foto]) : null
      );
    }
  }
    ...
    <div className="galeria grid-columns-5 p-3">
      {fotos.map((foto) => (
        <div className="fotos zoom" key={foto.id}>
          ...
          <p>{foto.alt}</p>
          <i
            className="bx bxs-heart corazon bx-lg"
            id={`${foto.id}`}
            onClick={() => favoritos(foto.id)}
          ></i>
        </div>
      ))}
    </div>
